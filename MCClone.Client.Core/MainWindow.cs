...
namespace MCClone
{
    public class MainWindow : GameWindow
    {
        ...
        public MainWindow() : base(new GameWindowSettings() { IsMultiThreaded = false }, new NativeWindowSettings() { API = OpenTK.Windowing.Common.ContextAPI.OpenGL, AutoLoadBindings = true, Size = new Vector2i(1280, 720), Profile = ContextProfile.Compatability })
        {
        ...
        protected override void OnResize(ResizeEventArgs e)
        {
            ...
            centerX = ClientRectangle.Size.X / 2;
            centerY = ClientRectangle.Size.Y / 2;
            Matrix4 projection = Matrix4.CreatePerspectiveFieldOfView((float)Math.PI / 4/* 0.9f*/, ClientRectangle.Size.X / (float)ClientRectangle.Size.Y, 1.0f, 6400f);
        ...
        private static void RenderWorld()
        {
            ...
            try
            {
                ...
                foreach (Chunk ce in chunks) { if (CliUtil.ShouldRenderChunk(ce) && ce != null && ce.Blocks != null) { foreach (Block bl in ce.Blocks.Values) { RenderCube(ce, bl); } RenderedChunks++; } };
